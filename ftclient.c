...
int ftclient_read_command(char* buf, int size, struct command *cstruct)
{
    ...
    // buf = command
    if (strcmp(buf, "servls") == 0)
    {
        strcpy(cstruct->code, "SLS");
    }
    else if (strcmp(buf, "fget") == 0)
    {
    ...
    else if (strcmp(buf, "fput") == 0)
    {
        strcpy(cstruct->code, "PUT");
    }
    else if (strcmp(buf, "servcd") == 0)
    {
        strcpy(cstruct->code, "SCD");
    }
    else if (strcmp(buf, "clils") == 0)
    {
        strcpy(cstruct->code, "LLS");
    }
    else if (strcmp(buf, "clicd") == 0)
    {
        strcpy(cstruct->code, "LCD");
    }
    else if (strcmp(buf, "servpwd") == 0)
    {
        strcpy(cstruct->code, "SPWD");
    }
    else if (strcmp(buf, "clipwd") == 0)
    {
        strcpy(cstruct->code, "LPWD");
    }
    else
    {//invalid
        return -1;
...
{
    if (strcmp(cmd.code, "LLS") == 0)
    {
        if (system("ls -l | tail -n+2") < 0)
            perror("error in system call ");
    }
    else if (strcmp(cmd.code, "LPWD") == 0)
    {
        if (system("pwd") < 0)
            perror("error in system call ");
    }
    else if (strcmp(cmd.code, "LCD") == 0)
    {
        //        char buff[50] = "cd ";
        //        strcat(buff, cmd.arg);
        //        if (system(buff) < 0)
